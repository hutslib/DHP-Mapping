...
void SuperPixel::addIndexByRange(int indRow, int colStart, int colEnd) {
  for (int k = colStart; k <= colEnd; k++) {
    pixel_indexes[current_ind++] =
        indRow * im_width +
        k;  // (im_height - indRow - 1) * im_width + k; // reverse save
  }
}
...
Eigen::MatrixXi get_label_to_color_matrix() {
  ...
  label_to_color << 128, 0,
      0,  // 1. building  // our 11 class  //TODO this conversion should be the
...
void show_unary_result(const cv::Mat& unary_img, const cv::Mat& raw_rgb_img,
                       cv::Mat& label_blended_img) {
  addWeighted(raw_rgb_img, 0.5, unary_img, 0.5, 0, label_blended_img);
}

void show_pixel_crf_result(Eigen::VectorXi pix_labels,
                           const cv::Mat& raw_rgb_img, cv::Mat& label_bw_img,
                           cv::Mat& label_color_img,
                           cv::Mat& label_blended_img) {
  int im_width = raw_rgb_img.cols;
  int im_height = raw_rgb_img.rows;
  Eigen::MatrixXi mat =
      Eigen::Map<MatrixXi_row>(pix_labels.data(), im_height, im_width);
  ...
  label_color_img =
      cv::Mat(cv::Size(im_width, im_height), CV_8UC3, cv::Scalar(0));
...
void show_superpixel_crf_result(const std::vector<SuperPixel*>& all_superpixels,
                                const Eigen::VectorXi& sp_labels,
                                const cv::Mat& raw_rgb_img,
                                cv::Mat& label_blended_img) {
  // assume input is >= 1 all_superpixels
  int im_width = all_superpixels[0]->im_width;
  int im_height = all_superpixels[0]->im_height;

  cv::Mat label_color_img(cv::Size(im_width, im_height), CV_8UC3,
                          cv::Scalar(0));
  for (int s = 0; s < all_superpixels.size(); s++) {
    ...
    for (int i : sp->pixel_indexes) {
      ...
      label_color_img.at<cv::Vec3b>(vertical_x, horizontal_y)[0] =
          color(2);  // b
      label_color_img.at<cv::Vec3b>(vertical_x, horizontal_y)[1] =
          color(1);  // g
      label_color_img.at<cv::Vec3b>(vertical_x, horizontal_y)[2] =
          color(0);  // r
    }
  }
  addWeighted(raw_rgb_img, 0.5, label_color_img, 0.5, 0, label_blended_img);
...
void read_superpixel_img(const std::string superpixel_img_file,
                         std::vector<SuperPixel*>&
                             all_superpixels) {  // this method is slow compared
                                                 // to bin, even exclude imread
  ...
  int im_width = sp_img.cols;
  int im_height = sp_img.rows;
  ...
  for (int y = 0; y < im_height; y++)
    for (int x = 0; x < im_width; x++) {
      all_superpixels_raw[sp_img.at<uint16_t>(y, x)]->pixel_indexes.push_back(
          y * im_width + x);
    }
  for (int i = 0; i < max_sp_num; i++) {
...
void read_superpixel_bin(const std::string superpixel_bin,
                         std::vector<SuperPixel*>& all_superpixels,
                         int im_width, int im_height) {
  // TODO move to config
  int max_sp_num = 170;
  int col_num = 2077;  // 72   // match max_sp_num in read_superpixel_img

  int16_t* frame_superpixels = get_frame_superpixels(
      max_sp_num, col_num);  // new int16_t[max_sp_num*col_num];
  std::ifstream fLables(superpixel_bin.c_str(),
  ...
  if (visualize_segment)
    clique_segment_img =
        cv::Mat(cv::Size(im_width, im_height), CV_8UC1, cv::Scalar(0));
  ...
  if (fLables.is_open()) {
    ...
    for (int i = 0; i < segmentCount; i++) {
      ...
      // 			std::cout << i << " pixel_block_num "
      // <<pixel_block_num << std::endl;
      int pixel_num = 0;
  ...
  // 		ROS_ERROR_STREAM("Cannot open bianry label file
  // "<<superpixel_bin);
  // 	}
}
