...
def create_label_ids_flat(ir_correction_file):
    ...
    labels = []  # {InstanceID, ClassID, PanopticID, MeshID, InfraredID, Name, Size}
    ...
    def set_label(name, size="M", increment_class=True, count=1, new_id=True):
        for i in range(count):
            if i >= len(letters):
                print("Warning: can only write %i (request is %i) suffixes " "using 'letters'." % (len(letters), i))
            ...
            if label_id < len(mesh_ids):
                ...
                print(
                    "Warning: id '%i' is larger than the maximum supported "
    ...
    id_counter[0] = id_counter[0] + 1
    set_label("SM_Laptop", size="S", count=2, increment_class=False, new_id=False)

    print("Created a labeling with %i instances and %i classes." % (id_counter[0], class_counter[0]))
    return labels  # noqa
...
class Labler:
    ...
    def __init__(self, ir_correction_file):
        ...
        self.labels = []  # {InstanceID, ClassID, PanopticID, MeshID, InfraredID, Name}
    ...
    def add_instance(self, mesh_names, class_name):
        ...
        if class_name in self.class_labels:
            self.class_labels[class_name][1] = self.class_labels[class_name][1] + 1
        else:
            self.class_labels[class_name] = [self.class_counter, 0]
            self.class_counter = self.class_counter + 1
        label["Name"] = class_name + "_" + str(self.class_labels[class_name][1])
...
def create_label_ids_large_flat(ir_correction_file):
    ...
    labler.add_instance(["pr_background_mountain_a_4_summer_16", "Ground_195"], "landscape")
    labler.add_instance(
        [
            "SM_window07_frame_14",
            "SM_window07_leaf01_22",
            "SM_window26_leaf01_8",
            "SM_window26_frame2_134",
            "SM_window26_frame3",
        ]
        + ["SM_window07_frame%i" % i for i in range(2, 8)]
        + ["SM_window07_leaf%i" % i for i in range(2, 15)]
        + ["SM_window26_leaf%i" % i for i in range(2, 5)],
        "window",
    )
    ...
    labler.add_instance(
        [
            ...
            "SM_Kitchen_KNOXHULT_Lamps3",
            "SM_Kitchen_KNOXHULT_Sink3",
            "SM_Kitchen_KNOXHULT_Tile_4",
            "SM_Kitchen_KNOXHULT_WallCabinet_Boxes3",
            "SM_Kitchen_KNOXHULT_WallCabinet_Doors3",
        ],
        "kitchen",
    )
    labler.add_instance(["SM_Refregerator_01_140"], "refregerator")
    labler.add_instance(["SM_Stand3", "SM_Spice_jar11", "SM_Spice_jar12", "SM_Spice_jar13"], "spices")
    ...
    labler.add_instance(["SM_Jar3", "SM_Spice_jar14", "SM_Spice_jar15"], "spices")
    ...
    labler.add_instance(["SM_Sofa_01_Fold_32", "SM_Pillow_Sofa_03_38", "SM_Pillow_Sofa_4", "SM_Pillow_Sofa_4"], "sofa")
    ...
    labler.add_instance(
        [
            "SM_TVSet_SM_MailUnitBdvE4102",
            "SM_TVSet_SM_SurroundSpeakers5",
            "SM_TVSet_SM_SurroundSpeakers6",
            "SM_TVSet_SM_centerpeaker2",
        ],
        "soundsystem",
    )
    labler.add_instance(
    ...
    labler.add_instance(
        ["Shoe_shelf_Hemnes3", "SM_Coat_hanger_2"] + ["SM_Hanger_%i" % i for i in range(23, 27)], "wardrobe"
    )
    labler.add_instance(["SM_Picture_140x100_74"], "picture")

    # Toilet
    labler.add_instance(["SM_Toilet_02_229", "SM_ToiletPaper_02_235", "SM_Toiletbrush_03_232"], "toilet")
    ...
    labler.add_instance(["SM_Door_80_7", "SM_handleMetal11", "SM_handleMetal12"], "door")
    labler.add_instance(["SM_Door_80_8", "SM_handleMetal13", "SM_handleMetal14"], "door")
    ...
    labler.add_instance(["SM_Washbasin_03_188", "SM_BathroomSinkMixer_02_105"], "basin")
    labler.add_instance(["SM_Mirror_01_76"], "mirror")
    labler.add_instance(["SM_Cosmetics_4_01_120", "SM_Cosmetics_04_01_111", "SM_Cosmetics_3_01_123"], "cosmetics")
    labler.add_instance(["SM_ToothbrushHolder_02_126", "SM_Cosmetics_04_02_114", "SM_Cosmetics_1_01_117"], "cosmetics")
    ...
    labler.add_instance(["SM_Bath_01_71", "SM_Shower_01_SM_Shower_01_79"], "bath")
    labler.add_instance(["SM_Branas_01_98"], "basket")
    labler.add_instance(["SM_WashingMachine_01_92"], "washingmachine")

    # Bedroom
    labler.add_instance(
        ["SM_P1sh12_%s" % i for i in [158, 174, 181, 184]] + ["SM_Hanger_%i" % i for i in range(14, 23)], "shelf"
    )
    ...
    labler.add_instance(
        ["SM_RackKallax_4x5_189", "SM_BoxDoorKallax_25", "SM_BoxFiella_6", "SM_BoxDoorKallax_26"], "shelf"
    )
    ...
    labler.add_instance(["SM_Notebook_01_keyboard_223", "SM_Notebook_01_top_226"], "pc")
    labler.add_instance(["SM_Door_80_6", "SM_handleMetal10", "SM_handleMetal9"], "door")
    ...
    labler.add_instance(
        [
            "SM_Pen_Stand_Tiera_01_v4",
            "SM_Marker_01_SM_Marker_01_Cap5",
            "SM_Marker_01_SM_Marker_01_Cap5",
            "SM_Marker_01_SM_Marker_9",
            "SM_Marker_01_SM_Marker_01_Cap6",
            "SM_Marker_01_SM_Marker_7",
    ...
    # Office
    labler.add_instance(["SM_Door_80_5", "SM_handleMetal8", "SM_handleMetal7"], "door")
    ...
    print("Created a labeling with %i instances and %i classes." % (len(labler.get_labels()), labler.class_counter))
            ...
            if label["InstanceID"] != previous_id:
                ...
                writer.writerow(
                    [
                        ...
                        label["MeshID"],
                        label["InfraredID"],
                        color_palette[0, label["MeshID"] * 4, 0],
                        color_palette[0, label["MeshID"] * 4, 1],
                        color_palette[0, label["MeshID"] * 4, 2],
                        label["Name"],
                        label["Size"],
                    ]
                )
...
def main_flat(get_ir_corrections, apply_mesh_labels, export_mesh_labels):
    ir_file = "/home/lukas/Documents/Datasets/flat_dataset/" "infrared_corrections.csv"
...
def main_large_flat(get_ir_corrections, apply_mesh_labels, export_mesh_labels):
    ir_file = "/home/lukas/Documents/Datasets/large_flat_dataset/" "infrared_corrections.csv"
