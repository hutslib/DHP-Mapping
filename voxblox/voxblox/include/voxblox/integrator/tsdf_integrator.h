...
class TsdfIntegratorBase {
 public:
  ...
  struct Config {
    ...
    float default_truncation_distance = 0.08;
 ...
 protected:
  ...
  inline bool isPointValid(const Point& point_C, const bool freespace_point,
    ...
    // std::cout << "in voxblox ispointvalid: min is :" << config_.min_ray_length_m
    //           << "max is: " << config_.max_ray_length_m << std::endl;
    // exit(0);
  ...
  bool updateTsdfVoxel(const Point& origin, const Point& point_G,
                       ...
                       TsdfVoxel* tsdf_voxel, float* sdf);
...
class MergedTsdfIntegrator : public TsdfIntegratorBase {
 public:
  ...
  MergedTsdfIntegrator(const Config& config, Layer<TsdfVoxel>* layer)
      : TsdfIntegratorBase(config, layer) {
    // std::cout << "===========\n merged tsdf \n min and max:"
    //           << config_.min_ray_length_m << " " << config_.max_ray_length_m
    //           << std::endl;
  }
