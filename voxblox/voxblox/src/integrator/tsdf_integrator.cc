...
TsdfIntegratorBase::TsdfIntegratorBase(const Config& config,
  ...
  if (config_.default_truncation_distance < 0.f) {
    float vs = layer_->voxel_size();
    config_.default_truncation_distance *= -vs;
    // std::cout << "default truncatuon distance is : " << config_.default_truncation_distance
    //           << std::endl;
  }
  // std::cout<<config_.print();
}
...
void TsdfIntegratorBase::setLayer(Layer<TsdfVoxel>* layer) {
  ...
  layer_ = layer;
  // std::cout << "voxel size herer lllllook: " << layer_->voxel_size();
  // exit(0);
...
bool TsdfIntegratorBase::updateTsdfVoxel(const Point& origin,
                                         ...
                                         TsdfVoxel* tsdf_voxel, float* return_sdf) {
...
void SimpleTsdfIntegrator::integrateFunction(const Transformation& T_G_C,
  ...
  while (index_getter->getNextIndex(&point_idx)) {
    ...
    while (ray_caster.nextRayIndex(&global_voxel_idx)) {
      ...
      float sdf;
      updateTsdfVoxel(origin, point_G, global_voxel_idx, color, weight, voxel, &sdf);
    }
  }
}
...
void MergedTsdfIntegrator::integratePointCloud(const Transformation& T_G_C,
                                               ...
                                               const Colors& colors,
                                               const bool freespace_points) {
  // std::cout << "in voxblox tsdf integrator" << std::endl;
...
void MergedTsdfIntegrator::bundleRays(
  ...
  while (index_getter->getNextIndex(&point_idx)) {
    ...
    const Point point_G = T_G_C * point_C;
    // std::cout<<"point_G: "<<point_G<<std::endl;
...
void MergedTsdfIntegrator::integrateVoxels(
  ...
  size_t map_size;
  if (clearing_ray) {
    // std::cout<<"clearing ray"<<std::endl;
    it = clear_map.begin();
...
void MergedTsdfIntegrator::integrateRays(
    ...
    const LongIndexHashMapType<AlignedVector<size_t>>::type& voxel_map,
    const LongIndexHashMapType<AlignedVector<size_t>>::type& clear_map) {
  std::cout << "integrate rays" << std::endl;
  ...
  if (config_.integrator_threads == 1) {
    ...
    std::cout << "thread: " << config_.integrator_threads << std::endl;
    std::list<std::thread> integration_threads;
    for (size_t i = 0; i < config_.integrator_threads; ++i) {
